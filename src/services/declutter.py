...
def parse_decision_from_analysis(analysis: str) -> str:
    """Parse the decision (keep/consider/discard) from AI analysis."""
    analysis_lower = analysis.lower()

    # Look for decision indicators
    if "🔴" in analysis or "捨棄" in analysis_lower:
        return "discard"
    elif "🟡" in analysis or "考慮" in analysis_lower:
        return "consider"
    elif "🟢" in analysis or "保留" in analysis_lower:
        return "keep"

    return "consider"  # Default to consider if unclear


def parse_item_name_from_analysis(analysis: str) -> str:
    """Extract item name from AI analysis."""
    # Try to find "物品識別" section
    lines = analysis.split("\n")
    for i, line in enumerate(lines):
        if "物品識別" in line or "這是" in line:
            # Get the text after the colon or the next line
            if "：" in line:
